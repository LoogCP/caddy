...
DOWNLOAD_URL=$(curl -fsSL "https://api.github.com/repos/${REPO}/releases/latest" | grep "browser_download_url" | grep "linux-${ARCH}\.tar\.gz" | head -n1 | sed -E 's/.*"browser_download_url": "([^"]+)".*/\1/')
...
# 2. 下载 Caddyfile 配置文件
wget -qO "$BASE_DIR/etc/caddy/Caddyfile" "https://raw.githubusercontent.com/${REPO}/main/Caddyfile"
...
# 3. 下载 init 脚本
wget -qO "$BASE_DIR/etc/init.d/caddy" "https://raw.githubusercontent.com/${REPO}/main/caddy.init"
...
# 4. 下载 ubus 脚本
wget -qO "$BASE_DIR/usr/bin/ubus.sh" "https://raw.githubusercontent.com/${REPO}/main/ubus.sh"
